...
def normalize_dates_in_text(text):
    ...
    # 날짜 패턴들 정의 (순서 중요: 더 구체적인 패턴을 먼저)
    patterns = [
        # YYYY.MM.DD 형식 (2023.07.21)
        (r'(\d{4})\.(\d{1,2})\.(\d{1,2})', lambda m: f"{m.group(1)}년 {int(m.group(2))}월 {int(m.group(3))}일"),
        # YY.MM.DD 형식 (23.07.21, 25.03.21) - 단어 경계 추가
        (r'\b(\d{2})\.(\d{1,2})\.(\d{1,2})\b', lambda m: f"20{m.group(1)}년 {int(m.group(2))}월 {int(m.group(3))}일"),
        # YYYY-MM-DD 형식
        (r'(\d{4})-(\d{1,2})-(\d{1,2})', lambda m: f"{m.group(1)}년 {int(m.group(2))}월 {int(m.group(3))}일"),
        # YY-MM-DD 형식
        (r'\b(\d{2})-(\d{1,2})-(\d{1,2})\b', lambda m: f"20{m.group(1)}년 {int(m.group(2))}월 {int(m.group(3))}일"),
        ...
        (r'(\d{4})년\s*(\d{1,2})월\s*(\d{1,2})일', lambda m: f"{m.group(1)}년 {int(m.group(2))}월 {int(m.group(3))}일"),
        # YY년 MM월 DD일 형식 - 뒤에 년이 붙지 않은 경우만
        (r'\b(\d{2})년(?!\d{2,4}년)\s*(\d{1,2})월\s*(\d{1,2})일', lambda m: f"20{m.group(1)}년 {int(m.group(2))}월 {int(m.group(3))}일"),
        # MM월 DD일 형식 (연도 추가) - 앞에 연도가 없는 경우만
        (r'(?<!\d{2,4}년\s{0,3})\b(\d{1,2})월\s*(\d{1,2})일', lambda m: f"{datetime.now().year}년 {int(m.group(1))}월 {int(m.group(2))}일"),
        # MM.DD 형식 (연도 추가) - 추가
        (r'\b(\d{1,2})\.(\d{1,2})\b(?!\d)', lambda m: f"{datetime.now().year}년 {int(m.group(1))}월 {int(m.group(2))}일"),
        # MM-DD 형식 (연도 추가) - 추가
        (r'\b(\d{1,2})-(\d{1,2})\b(?!\d)', lambda m: f"{datetime.now().year}년 {int(m.group(1))}월 {int(m.group(2))}일"),
    ]
...
"""
    ...
    async def search(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # 디버깅: 원본 쿼리와 정규화된 쿼리 로그
        normalized_query = normalize_dates_in_text(query)
        logger.info(f"검색 쿼리 - 원본: '{query}', 정규화: '{normalized_query}'")

        # 디버깅 메시지 추가
        debug_info = f"🔍 검색 정보:\n원본 쿼리: {query}\n정규화된 쿼리: {normalized_query}\n"

        status_message = await update.message.reply_text(f"⏳ '{query}'와(과) 유사한 메시지를 찾고 있습니다...\n\n{debug_info}")

        # 가장 유사한 메시지 5개 검색 (디버깅을 위해)
        distances, db_ids = self.search_engine.search(query, k=5)
        ...
        for i, db_id in enumerate(db_ids[:5]):
            ...
            if msg_data:
                ...
                debug_results += f"   원본: {text[:50]}...\n"
                debug_results += f"   정규화: {normalized_text[:50]}...\n"
        ...
        if not found_message:
            await context.bot.edit_message_text(
                text=f"❌ '{query}'에 대한 검색 결과 중 필터 조건에 맞는 메시지가 없습니다.\n\n{debug_results}",
                chat_id=status_message.chat_id, message_id=status_message.message_id
            )
            return
        ...
        message_text += f"💬 내용:\n{preview}\n\n"
        message_text += debug_results  # 디버깅 정보 추가
...
get_ipython().system('jupyter nbconvert --to python searchbot_ver0.5.0.ipynb')
